...
//   npm run lookup -- 11101 [--as-of 2026-09-15]
...
const { values, positionals } = parseArgs({ allowPositionals: true, options: { 'as-of': { type: 'string' } } });

try {
  if (positionals.length !== 1) throw new Error('Usage: npm run lookup -- <storeNumber> [--as-of YYYY-MM-DD]');
  ...
  const result = await lookup({ storeNumber: positionals[0], asOfDate: values['as-of'], correlationId: `cli-${Date.now()}` });
